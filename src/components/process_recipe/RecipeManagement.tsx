...
const RecipeFormModal: React.FC<RecipeFormModalProps> = ({ recipe, onClose, onSave }) => {
    ...
    const addProcess = () => {
        const newProcess: LocalProcessDto = {
            name: '', stepOrder: localProcesses.length + 1, description: '', temp: 0, ph: 0, doValue: 0, time: '',
                                                    ...
                                                    <Draggable key={process.tempId} draggableId={process.tempId} index={index}>
                                                        {(provided, snapshot) => (
                                                            <tr ref={provided.innerRef} {...provided.draggableProps} 
                                                                ...
                                                                <td><input className="form-control form-control-sm" type="number" step="0.1" value={process.doValue || ''} 
                                                                onChange={(e) => handleProcessChange(index, 'doValue', Number(e.target.value))} placeholder="%"/></td>
