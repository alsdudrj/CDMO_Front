...
import { PageContainer, StyledCard, LeftImage, FormWrapper, GenderLabel, 
  AddressGroup, AddressButton, SubmitButton, SocialButton, FooterLink, FooterLinks } from "../styled/Member.styles";
import { useNavigate } from "react-router-dom";

import {FontAwesomeIcon} from "@fortawesome/react-fontawesome";
import{ faGoogle, faInstagram, faKakaoTalk, faFacebookF } from "@fortawesome/free-brands-svg-icons";
...
//인스타 회원가입(소셜 로그인) 버튼 클릭시에 
    ...
    const handleChange = (e:ChangeEvent<HTMLInputElement & HTMLSelectElement>) => {
        ...
        //회원가입 Form에서 input select 둘다 이 함수 하나로 처리하려는 의도
        const {name, value} = e.target; //실제로 값이 바뀐 DOM요소 그안에 여러속성이 있는데 그중에서 두개만 뽑아 쓰려고 구조분해
        setForm((prev) => ({...prev, [name]:value})); //useState로 만든 form 상태를 바꾸는 함수
        //...prev : 기존 값들을 그대로 복사하고 | [name]:value : 방금 바뀐 그 한 필드만 덮어씀
    };

    //성별 선택용 함수
    const handleGenderChange = (e:ChangeEvent<HTMLInputElement>) => {
        setForm((prev) => ({...prev, gender:e.target.value as Gender}));
        //이 값이 분명히 젠더 타입이야 라고 강제로 캐스팅할때 as 사용
    };
    ...
    const handleAddressSearch = () => {
        ...
        */
            alert("주소검색 스크립트 로딩중 입니다. 잠시후 다시 시도해 주세요");
            return;
        }

        new window.daum.Postcode({ //카카오에서 제공하는 우편번호 검색 팝업 객체를 하나 생성하는 코드
            //가장 중요한 옵션이 oncomplete라는 콜백
            oncomplete: (data: any) => { //oncomplete는 검색 -> 주소선택 -> 확인 할때 실행, 그리고 그때 data라는 객체를 함께 넘겨준다.
                const fulladdr = data.address; //기본주소
                setForm((prev) => ({...prev, address: fulladdr}));
            },
        }).open(); //옵션을 가진 우편번호 검색 팝업을 생성하고 바로 화면에 띄워라
        /*
        why 이렇게 복잡하게 써 => 언제 끝날지 모르는 작업이 많다.
